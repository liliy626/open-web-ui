...
from open_webui.campus.tenants import env_campus_config, get_campus_config_for_user
...
def get_seed_campus_apps(campus: dict[str, Any] | None = None) -> list[CampusAgentApp]:
    campus = campus or env_campus_config()
    ragflow = campus.get('ragflow') or {}
    fastgpt = campus.get('fastgpt') or {}
    school_id = campus.get('school_id') or 'meilanhu_middle_school'
    ...
    return [
        ...
        CampusAgentApp(
            ...
            entry_url=ragflow.get('web_url') or 'http://localhost:9222',
            display_mode=CampusAppDisplayMode.IFRAME,
            category='knowledge',
            category_label='知识与依据',
            icon='note',
            badge='RAGFlow',
            api_base_url=ragflow.get('base_url'),
            sort_order=20,
        ),
        CampusAgentApp(
            ...
            entry_url=fastgpt.get('entry_url') or 'http://localhost:3006',
            display_mode=CampusAppDisplayMode.IFRAME,
            category='agents',
...
async def get_campus_apps(
    ...
    return sorted(
        [app for app in get_seed_campus_apps(campus) if app.enabled],
